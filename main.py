...
WELCOME_MESSAGE = (
    ...
    f"⚡ Live Signals & Precision Key Levels ({STRATEGY_DISPLAY_NAME}).\n"
    "📊 Real-Time Strikes for Option Buyers & Sellers.\n"
    "🎯 Accurate Expiry Day Hero-Zero Setups.\n\n"
...
# FOCUS EXCLUSIVELY ON NIFTY 50
INDEX_WATCHLIST = ["NIFTY 50"]
YF_TICKERS = {"NIFTY 50": "^NSEI"}
BACKUP_FILE = "active_trades_nifty_momentum.json"
MONTHLY_LEDGER_FILE = "monthly_trades_ledger_nifty.json"
...
# --- INTERNAL LEVEL ENGINE ---
def calculate_algo_levels(index_name):
    try:
        ...
        if not df_daily.empty:
            ...
            buy_trigger = close + (diff * 1.1 / 2.0)
            key_res = close + (diff * 1.1 / 4.0)
            key_sup = close - (diff * 1.1 / 4.0)
            sell_trigger = close - (diff * 1.1 / 2.0)
...
algo_levels["NIFTY 50"] = calculate_algo_levels("NIFTY 50")
...
def generate_detailed_pdf_report(filename, title_text, subtitle_text, logs_to_print):
    ...
    elements.append(Paragraph(f"SAS CAPITAL MARKET - {title_text}", title_style))
...
exp_note = f"🔥 TODAY IS NIFTY EXPIRY ({current_expiry_str})!" if is_today_expiry else f"• Next Expiry: {current_expiry_str}"
send_telegram_alert(
    f"🚀 NIFTY 50 SCANNER ACTIVATED\n\n"
    f"⚡ Strategy: {STRATEGY_DISPLAY_NAME}\n"
...
while True:
    try:
        ...
        if not pivots_alert_sent and current_time >= pivot_alert_time:
            ...
            lvl = algo_levels.get("NIFTY 50")
            if lvl:
                p_msg = (
                    f"📐 DAILY KEY LEVELS (09:05 AM IST)\n"
                    f"📅 Date: {today_date_str} | Strategy: {STRATEGY_DISPLAY_NAME}\n\n"
                    f"🔹 NIFTY 50 (Prev Close: {lvl['Prev_Close']})\n"
                    f"• Bullish Breakout Zone: Above {lvl['Buy_Trigger']:.2f}\n"
                    f"• Major Resistance: {lvl['Key_Res']:.2f}\n"
                    f"• Major Support: {lvl['Key_Sup']:.2f}\n"
                    f"• Bearish Breakdown Zone: Below {lvl['Sell_Trigger']:.2f}\n"
                )
        ...
        # 2. 03:40 PM MARKET CLOSE (PDF REPORTS & EMAIL)
        if current_time >= shutdown_time:
            ...
            summary = (
                ...
                f"• Today's Signals Logged: {len(today_records)}\n"
                f"• Targets / Locks Achieved: {wins}\n"
                f"• Stop Losses Hit: {trade_stats['sl_hits']}\n"
                f"• Early Risk Cuts: {trade_stats['early_cuts']}\n"
                f"🏆 Win Rate: {win_rate:.1f}%\n"
                f"📚 Total Month Trades Logged: {len(all_ledger_records)}\n\n"
            ...
            # Monthly Expiry Ledger PDF (If Monthly Expiry Completed)
            if is_monthly_expiry:
                ...
                send_telegram_document(pdf_monthly, caption=f"📚 Official Monthly Expiry Performance Ledger ({month_str})")
                send_email_with_pdf(pdf_monthly, f"NIFTY 50 Monthly Expiry Ledger - {month_str}", f"Attached full cumulative trade records up to {today_date_str}.")
        ...
        if raw_data is not None and not raw_data.empty:
            row = raw_data[raw_data['index'] == "NIFTY 50"]
            if not row.empty:
                current_price = float(str(row['last'].values[0]).replace(',', ''))

                if "NIFTY 50" not in prev_close_dict and 'previousClose' in row.columns:
                    prev_close_dict["NIFTY 50"] = float(str(row['previousClose'].values[0]).replace(',', ''))
                ...
                if is_today_expiry and not hero_zero_sent and current_time >= hero_zero_time:
                    ...
                    hz_msg = (
                        ...
                        f"• Buy Strike: *{atm + step} CE* (Expected Range: ₹15 - ₹35)\n"
                        f"• Strict SL: ₹6.00 | T1: ₹50.00 | T2: ₹80.00+\n\n"
                        f"🎯 *PUT OPTION (On Downside Spike):*\n"
                        f"• Buy Strike: *{atm - step} PE* (Expected Range: ₹15 - ₹35)\n"
                        f"• Strict SL: ₹6.00 | T1: ₹50.00 | T2: ₹80.00+\n\n"
                        f"⚡ *Rule:* 02:00 PM-ന് ശേഷമുള്ള അതിവേഗ മൊമെന്റത്തിൽ ട്രേഡ് ചെയ്യുക. T1-ൽ ക്യാപിറ്റൽ എടുത്തു ട്രേഡ് റിസ്ക്-ഫ്രീ ആക്കുക!"
                    )
                    send_telegram_alert(hz_msg)
                    hero_zero_sent = True

                # --- ACTIVE POSITION TRACKING & TIME LOGGING ---
                ...
                if trade is not None:
                    ...
                    if trade['type'] == 'BUY':
                        ...
                        # 1. Early Cut (Reversal below support level)
                        ...
                        # 2. Reversal Profit Lock (Gained 30+ pts, retraces to 10)
                        ...
                        # 4. Target 1 Hit -> Trail SL to Entry
                ...
                # --- ZERO-DELAY SIGNAL DETECTION ---
                lvls = algo_levels.get("NIFTY 50")
                if can_take_trades and active_trades["NIFTY 50"] is None and lvls is not None:
                    buy_trigger = lvls['Buy_Trigger']
                    sell_trigger = lvls['Sell_Trigger']
                    key_res = lvls['Key_Res']
                    key_sup = lvls['Key_Sup']
                ...
                if now.minute == 0 and now.hour != last_heartbeat_hour and (9 <= now.hour <= 15):
                    ...
                    prev_c = prev_close_dict.get("NIFTY 50", current_price)
                    diff = current_price - prev_c
                    pct = (diff / prev_c) * 100 if prev_c != 0 else 0.0
                    hb_msg = f"💓 HOURLY STATUS ALERT\n⏰ Time: {current_time_str} IST\n\n• NIFTY 50: {current_price:.2f} ({diff:+.2f} | {pct:+.2f}%)\n"
